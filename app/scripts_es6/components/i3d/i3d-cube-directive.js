...
//   Camera3d.actor[pid] if pid is defined, and as a child of webgl scene 
//   otherwise (i.e. if root)
// * NOTE: all work constructing the node (THREE.js Object3d) is done 
//   in pre-link function since pre-links are run after controller constructor
//   and in root-to-leaf order so that correct relative transforms are computed 
//   in webgl<br>
        ...
        */
        if(textureurl){
          ...
          if(/(_png|_jpg|_gif|_bmp)/.test(textureurl)){
            ...
            // so relative transforms are correctly applied in the hierarchy<br>
            // * NOTE: use ```window[textureurl]``` which converts string to 
            //   var name of a THREE.js texture
            if(textureurl){
